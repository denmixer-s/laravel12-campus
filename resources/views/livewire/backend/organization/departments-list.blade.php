<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 class="text-3xl font-bold text-slate-800 mb-2">Department Management</h1>
                        <p class="text-slate-600">Manage departments and work units across divisions</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm text-slate-600">With Users</p>
                        <p class="text-2xl font-bold text-slate-800">{{ $stats['with_users'] }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm text-slate-600">With Documents</p>
                        <p class="text-2xl font-bold text-slate-800">{{ $stats['with_documents'] }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            <input wire:model.live.debounce.300ms="search" type="text"
                                placeholder="Search departments by name, code, description, division, faculty, or university..."
                    ...
                    <div class="flex flex-col lg:flex-row gap-4">
                        ...
                        <div class="lg:w-56">
                            <select wire:model.live="divisionFilter"
                                class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                {{ !$facultyFilter ? 'disabled' : '' }}>
                                @foreach ($divisionFilterOptions as $value => $label)
                                    <option value="{{ $value }}">{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- Clear Filters -->
                        @if ($search || $universityFilter || $facultyFilter || $divisionFilter || $statusFilter)
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
            @if ($departments->count() > 0)
                ...
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50">
                    <div class="grid grid-cols-12 gap-4 items-center text-sm font-medium text-slate-700">
                        ...
                        <div class="col-span-3">Hierarchy Path</div>
                        <div class="col-span-2 text-center">Users & Documents</div>
                            ...
                            <div class="grid grid-cols-12 gap-4 items-center">
                                    ...
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div class="flex-shrink-0">
                                            <div class="w-12 h-12 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center">
                                                <span class="text-white font-bold text-lg">{{ substr($department->name, 0, 1) }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if ($department->description)
                                                <p class="mt-1 text-xs text-slate-600 line-clamp-2">
                                                    {{ $this->getContentPreview($department->description) }}
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div class="col-span-3">
                                    <div class="text-xs text-slate-600 space-y-1">
                                        <div class="flex items-center">
                                            ...
                                            <span class="truncate">{{ $department->division->faculty->university->name }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <svg class="w-3 h-3 text-slate-400 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                                            </svg>
                                            <span class="truncate">{{ $department->division->faculty->name }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <svg class="w-3 h-3 text-slate-400 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path>
                                            </svg>
                                            <span class="truncate">{{ $department->division->name }}</span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="flex flex-col items-center space-y-2">
                                        ...
                                        @if ($department->documents_count > 0)
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800"
                                                ...
                                                {{ $department->documents_count }} docs
                                            </span>
                                        @endif

                                        @if ($department->users_count === 0 && $department->documents_count === 0)
                                            <span class="text-xs text-slate-400">No users or docs</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="col-span-1">
                                    <div class="flex justify-center">
                                        @php $statusBadge = $this->getStatusBadge($department->is_active) @endphp
                                ...
                                <!-- Sort Order -->
                                <div class="col-span-1">
                                    <div class="text-sm text-slate-900"># {{ $department->sort_order }}</div>
                                    <div class="text-xs text-slate-500">{{ $department->created_at->format('M d, Y') }}</div>
                                </div>
                                ...
                                <div class="col-span-2">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        @if ($this->canUserDeleteDepartment($department))
            ...
            @else
                ...
                <div class="p-12 text-center">
                    ...
                    <h3 class="text-lg font-medium text-slate-900 mb-2">No departments found</h3>
                    <p class="text-slate-500 mb-4">
                        {{ $search || $universityFilter || $facultyFilter || $divisionFilter || $statusFilter ? 'Try adjusting your search criteria.' : 'Get started by creating your first department.' }}
                    </p>
                    @can('create', App\Models\Department::class)
                        <button wire:click="createDepartment"
        ...
        @if ($confirmingDepartmentDeletion)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
                wire:key="delete-modal-{{ $departmentToDelete }}"
                ...
                <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl" @click.stop>
                    <div class="flex items-center mb-4">
                        ...
                        <h3 class="ml-3 text-lg font-medium text-gray-900">Delete Department</h3>
                    </div>

                    <div class="mb-6">
                        <p class="text-sm text-gray-600 mb-3">
                            Are you sure you want to delete the department <strong
                                class="text-gray-900">"{{ $departmentToDeleteName }}"</strong>?
                        </p>
                        <div class="bg-red-50 border border-red-200 rounded-lg p-3">
                            <p class="text-sm text-red-800">
                                <strong>⚠️ Warning:</strong> This action cannot be undone and will permanently remove
                                this department and all its associated data.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="button"
                            ...
                            <span wire:loading.remove wire:target="deleteDepartment">Delete Department</span>
                            <span wire:loading wire:target="deleteDepartment" class="flex items-center">
        ...
        <!-- Loading Overlay -->
        <div wire:loading.flex wire:target="search,universityFilter,facultyFilter,divisionFilter,statusFilter,perPage,sortBy"
        ...
        @if (app()->environment('local'))
            <div class="fixed bottom-4 right-4 bg-black bg-opacity-75 text-white p-3 rounded-lg text-xs max-w-sm">
                ...
                <div>confirmingDepartmentDeletion: {{ $confirmingDepartmentDeletion ? 'true' : 'false' }}</div>
                <div>departmentToDelete: {{ $departmentToDelete ?? 'null' }}</div>
                <div>departmentToDeleteName: {{ $departmentToDeleteName ?? 'null' }}</div>
                <div>Current Filters:
                    {{ json_encode(['search' => $search, 'university' => $universityFilter, 'faculty' => $facultyFilter, 'division' => $divisionFilter, 'status' => $statusFilter]) }}
                </div>
            </div>
        @endif
        ...
        <script>
            document.addEventListener('livewire:initialized', () => {
                console.log('Livewire initialized for DepartmentsList component');

                Livewire.on('departmentDeleted', (event) => {
                    console.log('Department deleted event received:', event);
                });
            });

            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape' && @json($confirmingDepartmentDeletion)) {
            ...
            document.addEventListener('click', function(e) {
                ...
                // Debug delete department button click
                if (e.target.closest('[wire\\:click="deleteDepartment"]')) {
                    console.log('DELETE DEPARTMENT BUTTON CLICKED!');
                    console.log('Button element:', e.target.closest('[wire\\:click="deleteDepartment"]'));
                }
            });

            // Additional debug for Livewire calls
            document.addEventListener('livewire:request', (event) => {
                if (event.detail.request.payload.updates && event.detail.request.payload.updates[0] && event.detail.request.payload.updates[0].method === 'deleteDepartment') {
                    console.log('Livewire deleteDepartment request started');
                }
            });
