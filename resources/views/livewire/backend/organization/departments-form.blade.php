    ...
    <div class="container mx-auto px-4 py-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-slate-600">
                            {{ $isEdit ? 'Update department information' : 'Add a new department to a division' }}
                        </p>
                        @if ($hierarchyPath && $hierarchyPath !== 'No division selected')
                            <div class="mt-2">
        ...
        <form wire:submit.prevent="save">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                        <h2 class="text-lg font-semibold text-slate-800 mb-4">Organization Hierarchy</h2>
                        ...
                        <div class="mb-4">
                            ...
                            @if (!$university_id)
                                <p class="mt-1 text-xs text-slate-500">Please select a university first</p>
                            @endif
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="division_id"
                                    ...
                                    class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('division_id') border-red-500 @enderror"
                                    {{ !$faculty_id ? 'disabled' : '' }}>
                            ...
                            @if (!$faculty_id)
                                <p class="mt-1 text-xs text-slate-500">Please select a faculty first</p>
                            @endif
                            @error('division_id')
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                Lower numbers appear first. Auto-calculated based on division.
                            </p>
                            @error('sort_order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                        ...
                        <div class="mb-4">
                            ...
                            @endif
                            <p class="mt-1 text-xs text-slate-500">
                                Must be unique within the selected division
                            </p>
                            @error('slug')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div class="space-y-6">
                    ...
                    @if ($divisionInfo)
                        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                            <h3 class="text-lg font-semibold text-slate-800 mb-4">Division Information</h3>

                            <div class="space-y-3">
                                <div>
                                    <p class="text-sm font-medium text-slate-700">University</p>
                                    <p class="text-sm text-slate-600">{{ $divisionInfo->faculty->university->name }}</p>
                                </div>

                                <div>
                                    <p class="text-sm font-medium text-slate-700">Faculty</p>
                                    <p class="text-sm text-slate-600">{{ $divisionInfo->faculty->name }}</p>
                                </div>

                                <div>
                                    <p class="text-sm font-medium text-slate-700">Division</p>
                                    <p class="text-sm text-slate-600">{{ $divisionInfo->name }}</p>
                                ...
                                @if ($divisionInfo->description)
                                    <div>
                                        <p class="text-sm font-medium text-slate-700">Description</p>
                                        <p class="text-xs text-slate-600">{{ Str::limit($divisionInfo->description, 100) }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">
                                        {{ $is_active ? 'Department is currently active' : 'Department is currently inactive' }}
                                    </p>
                                </div>
                                <button type="button"
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="text-sm text-blue-700 mt-1 space-y-1">
                                    <p>• Select University → Faculty → Division in order</p>
                                    <p>• Code and slug must be unique per division</p>
                                    <p>• Sort order determines display order</p>
                                    <p>• Departments contain users and documents</p>
                                    @if (!$isEdit)
        ...
        <script>
            document.addEventListener('livewire:initialized', () => {
                console.log('DepartmentsForm component initialized');
            });
